...
4. Settings タブの Redirect URIs に開発用（`http://localhost:6606/` など）と
...
- 永続化（`saveVaultBytes`）が失敗した場合はメモリ上の credentials を
  **旧パスワードに戻してから** throw する。戻さないと「変更できなかった」
  と表示されたのに、以後のエントリ編集が新パスワードで再暗号化して保存する
  という不整合（利用者が知らないパスワードでの保存）が起きるため。
  逆に永続化が成功した後の後始末（生体認証失効・バックアップメタ更新）の
  失敗は throw せずログに留める — 変更自体は成功しており、失敗として
  報告するほうが害が大きい（残った生体認証レコードは旧パスワードを
  ラップしているだけなので無害）
...
- Dropbox 同期との相互作用: リモートに旧パスワードで暗号化された
  ...
  `InvalidPasswordError` を検知して案内を出すが、正しい対処は端末によって
  異なる: **変更した端末**では「Dropbox アプリでこのアプリ専用フォルダ内の
  `vault.kdbx` を削除してから再同期」（削除後はダウンロードが `null` になり
  マージがスキップされ、新パスワードの内容がそのままアップロードされる）。
  **変更していない別の端末**では削除させず「Dropbox の `vault.kdbx` を
  新パスワードで『バックアップから復元』してから同期」と案内する
  （削除すると変更した端末がアップロードした新しい保管庫を消してしまう
  ため）。利用者が App フォルダの中身を直接確認・操作できることは、
  そもそも Dropbox を選んだ理由の一つでもある
...
> `node scripts/serve-with-headers.mjs`（ポート6506、CSP付き配信）を使うこと。
...
| `./docker.sh shell` | コンテナ shell（ポート 6606/6506 公開） |
